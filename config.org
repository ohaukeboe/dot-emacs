...
The configuration for pulling mail using mbsyncrc is in [[file:dotfiles/mbsyncrc.conf]] and for sending mail using msmtp is in [[file:dotfiles/msmtprc.conf]].

To initialize the maildir run:
#+begin_src sh
  mkdir ~/.mail ~/.mail/ifi ~/.mail/proton
  mu init --maildir=~/.mail --my-address=oskah@uio.no --my-address=ohaukeboe@pm.me
  mu index
  mbsync -a
#+end_src
