...
#+begin_src elisp
  (use-package emacs
    ...
    :custom-face
    (default ((t :family "Roboto Mono" :height 140)))
    (fixed-pitch ((t :family "Roboto Mono" :height 1.1)))
...
#+end_example
to =.mbsyncrc= and put the certificate generated by ~openssl s_client -starttls imap -connect 127.0.0.1:1143 -showcerts~ in =~/.cert/protonmail.crt=, i.e. the lines between (and including) =-----BEGIN CERTIFICATE-----= and =-----END CERTIFICATE-----=
...
#+begin_src sh
  mkdir ~/.mail ~/.mail/ifi ~/.mail/proton
  mu init --maildir=~/.mail/proton --my-address=ohaukeboe@pm.me
  mu init --maildir=~/.mail/ifi --my-address=oskah@uio.no
  mu index
  mbsync -a
#+end_src
