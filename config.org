...
#+begin_src elisp
  (use-package consult
    ...
    (defvar +consult-special-filter "\\`\\*dashboard\\*\\'")
    ...
    (defun oh/insert-into-list (list el n)
      "Insert into list LIST an element EL at index N.

      If N is 0, EL is inserted before the first element.

      The resulting list is returned.  As the list contents is
  mutated in-place, the old list reference does not remain valid."
      (let* ((padded-list (cons nil list))
             (c (nthcdr n padded-list)))
        (setcdr c (cons el (cdr c)))
        (cdr padded-list)))

    (oh/insert-into-list 'consult-buffer-sources 4 '+consult-source-special))
    ;; (defun oh/add-element-after (list element new-element)
    ;;  "Add NEW-ELEMENT after ELEMENT in the LIST."
    ;;  (when-let ((pos (seq-position list element)))
    ;;    (setcdr (nthcdr pos list) (cons new-element (nthcdr (1+ pos) list)))))

    ;; (oh/add-element-after 'consult-buffer-sources 'consult--source-buffer '+consult-source-special))




    ;; (add-to-list 'consult-buffer-sources '+consult-source-special))
#+end_src
