...
- [~] T045 Walk `quickstart.md` §2 manually in a scratch tracker — all eleven checks, including the cycle case and the two-keystroke and under-15-seconds measurements (SC-002, SC-003). **Partly done**: the behavioural checks (1–3, 7, 11) were run headlessly against the *real* `bd` and this repository's own tracker rather than the fake one — marks, body lines, `beads-goto-parent`, `beads-go-back` and the call counts all confirmed, and R11 was found that way. The keystroke count (SC-002) and the under-15-seconds chain (SC-003) need a human at an interactive Emacs and are outstanding
...
- [ ] T048 Deploy with `sudo nixos-rebuild switch --flake .#<hostname>` and confirm the section behaves in the real Emacs, against this repository's own tracker, before closing anything
- [ ] T049 Close the beads issues from T001 with `bd close` once T045 and T048 are done
