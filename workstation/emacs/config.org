...
=wgrep= makes it possible to edit the results from the grep command, just like you can in occur-mode. This is built into emacs 31, so it can be safely removed once I update.
...
#+begin_src elisp
  (use-package wgrep
    ...
    (when (>= emacs-major-version 31)
      (warn "wgrep configuration can be safely removed in Emacs 31+")))
#+end_src
