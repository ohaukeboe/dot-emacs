...
#+begin_src elisp
  (use-package vterm
    ...
    (defun vterm-project-buffer-p (buffer)
      "Return non-nil if BUFFER is a project vterm buffer."
      (and (buffer-live-p buffer)
           (project-current nil (buffer-local-value 'default-directory buffer))
           (string-prefix-p (with-current-buffer buffer
    ...
    (defun project-toggle-vterm (&optional n)
      ...
      (let* ((project (project-current t))
             ...
             (prefix (project-prefixed-buffer-name "vterm"))
             (project-vterms (seq-filter (lambda (b)
                                           (and (buffer-live-p b)
                                                (string-prefix-p prefix (buffer-name b))))
                                         vterms))
             (default-directory root)
             (vterm-buffer-name prefix)
             (num (cond ((numberp n) n)
                        ((consp n) (1+ (length project-vterms)))
                        (t nil))))
        (cond ((numberp num) (push (vterm num) vterms))
              ((null project-vterms) (push (vterm 1) vterms))
    ...
    (defun toggle-vterm (&optional n)
      ...
      (let* ((default-directory (or (vc-root-dir) default-directory))
             (global-vterms (seq-remove 'vterm-project-buffer-p vterms))
             (num (cond ((numberp n) n)
                        ((consp n) (1+ (length global-vterms)))
                        (t nil))))
        (cond ((numberp num) (push (vterm num) vterms))
              ((null global-vterms) (push (vterm 1) vterms))
              ((seq-contains-p global-vterms (current-buffer))
