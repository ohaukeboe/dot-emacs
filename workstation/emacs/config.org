...
Nothing here starts a service; =direnv= only enters an environment.  The
services a kind declares are started by their own supervisor -- =just up= for
services-flake, =devenv up= for devenv.
