...
#+begin_src elisp
  (use-package vterm
    ...
    :bind
    ("C-z" . toggle-vterm)
    ("M-z" . project-toggle-vterm)
    ("C-c z" . my/consult-vterm)
    (:map vterm-mode-map
          ("C-z" . toggle-vterm)
          ("M-z" . project-toggle-vterm)
          ("M-y" . vterm-yank-pop)
    ...
    :preface
    (defun vterm-buffer-project-root (buffer)
      "Return project root for BUFFER if it's a vterm in a project, else nil."
    ...
    (defun vterm-buffers-in-project (root)
      "Return live vterm buffers whose project root equals ROOT."
      (let ((root (file-name-as-directory root)))
        (seq-filter (lambda (b)
                      (when-let ((b-root (vterm-buffer-project-root b)))
                        (equal (file-name-as-directory b-root) root)))
                    (buffer-list))))

    (defun vterm-buffers-global ()
      "Return live vterm buffers not associated with any project."
      (seq-filter (lambda (b)
                    (and (eq (buffer-local-value 'major-mode b) 'vterm-mode)
                         (not (string-prefix-p "*claude-code[" (buffer-name b)))
                         (not (vterm-buffer-project-root b))))
                  (buffer-list)))

    (defun vterm--spawn-fresh ()
      "Spawn a new vterm buffer with a unique name."
      (let ((vterm-buffer-name (generate-new-buffer-name "*vterm*")))
        (vterm)))

    (defun project-toggle-vterm (&optional n)
      (interactive "P")
      (let* ((project (project-current t))
             (root (project-root project))
             (project-vterms (vterm-buffers-in-project root))
             (default-directory root))
        (cond ((numberp n) (vterm n))
              ((or (consp n) (null project-vterms)) (vterm--spawn-fresh))
              ((memq (current-buffer) project-vterms)
               (switch-to-buffer (car (seq-difference (buffer-list) project-vterms))))
              (t (switch-to-buffer (car (seq-intersection (buffer-list) project-vterms)))))))

    (defun toggle-vterm (&optional n)
      (interactive "P")
      (let* ((default-directory (expand-file-name "~"))
             (global-vterms (vterm-buffers-global)))
        (cond ((numberp n) (vterm n))
              ((or (consp n) (null global-vterms)) (vterm--spawn-fresh))
              ((memq (current-buffer) global-vterms)
               (switch-to-buffer (car (seq-difference (buffer-list) global-vterms))))
              (t (switch-to-buffer (car (seq-intersection (buffer-list) global-vterms)))))))
    ...
    (defun my/project-consult-vterm ()
      ...
      (let* ((project (project-current))
             ...
             (vterm-bufs (and root (vterm-buffers-in-project root)))
    ...
    (defun my/consult-vterm ()
      "Consult-style picker for global (non-project) vterm buffers."
      (interactive)
      (require 'consult)
      (let* ((vterm-bufs (vterm-buffers-global))
             (source `(:name "Global VTerm Buffers"
                             :narrow ?v
                             :category buffer
                             :action ,#'switch-to-buffer
                             :items ,(mapcar #'buffer-name vterm-bufs)
        ...
        (if vterm-bufs
            (consult-buffer (list source))
          (user-error "No global vterm buffers"))))

    :init
    (add-to-list 'project-switch-commands '(project-toggle-vterm "Vterm" "t") t)
...
#+begin_src elisp
  (use-package gptel-magit
    ...
    :custom
    (gptel-magit-model "openrouter/google/gemini-3.1-flash-lite")
     ...
     `(((name . "mcp-nixos")
       ...
       ((name . "github-mcp")
        ...
        (env . (((name . "GITHUB_PERSONAL_ACCESS_TOKEN")
                 (value .,(auth-source-pick-first-password :host "api.github.com"))))))
       ;; TODO: uncomment this when it is publically available
       ;; ((name . "kagimcp")
       ;;  (type . "stdio")
       ;;  (command . "caveman-shrink")
       ;;  (args . ("uvx" "kagimcp"))
       ;;  (env . (((name . "KAGI_API_KEY")
       ;;           (value .,(auth-source-pick-first-password :host "kagi.com"))))))
       ((name . "elisp-dev-mcp")
        (type . "stdio")
        (command . "caveman-shrink")
        (args . (,(expand-file-name "~/.config/emacs/emacs-mcp-stdio.sh")
...
#+begin_src elisp
  (use-package claude-code-ide
    ...
    (defun my/claude-code-ide-tmux-wrap (cmd)
      "Wrap CMD with `tmux new-session -A -s <project-name>'.
  Attaches to an existing tmux session named after the project if one
  exists, otherwise creates a new one running CMD."
      (format "tmux new-session -A -s %s %s"
              (shell-quote-argument (my/claude-code-ide--tmux-session-name))
              (shell-quote-argument cmd)))

    (advice-add 'claude-code-ide--build-claude-command
                :filter-return #'my/claude-code-ide-tmux-wrap))
#+end_src
