...
ARG, start a new named instance here."
      (interactive "P")
    ...
    (defun my/claude-code-ide--tmux-session-name ()
      "Return a tmux-safe session name derived from the current project."
      (let* ((proj (project-current))
             (raw (if proj (project-name proj) "claude")))
        (replace-regexp-in-string "[^A-Za-z0-9_-]" "_" raw))))
#+end_src
