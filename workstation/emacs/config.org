...
#+begin_src sh
  mkdir ~/.mail ~/.mail/ifi ~/.mail/proton ~/.mail/knowit ~/.mail/knowit-drift
  mu init --maildir=~/.mail --my-address=oskah@uio.no --my-address=ohaukeboe@pm.me --my-address=oskar.haukeboe@knowit.no --my-address=drift@knowit.no
  mu index
  mbsync -a
#+end_src
    ...
    (setq mu4e-contexts
          (list
           ...
           (make-mu4e-context
            ...
            (lambda (msg)
              (when msg
                (string-prefix-p "/knowit-drift" (mu4e-message-field msg :maildir))))
            :vars '((user-mail-address . "drift@knowit.no")
                    (user-full-name . "Drift")
                    (mu4e-sent-folder . "/knowit-drift/Sent")
                    (mu4e-trash-folder . "/knowit-drift/Trash")
                    (mu4e-drafts-folder . "/knowit-drift/Drafts")
                    (mu4e-refile-folder . "/knowit-drift/Archive")
                    (mu4e-compose-signature . nil)
                    (smtpmail-smtp-user . "drift@knowit.no"))))))
#+end_src
